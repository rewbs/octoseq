...
export async function runMir(
  ...
  const computeMel = async (
    useGpu: boolean
  ): Promise<{ mel: MelSpectrogram; usedGpu: boolean; gpuMs?: number; cpuExtraMs: number }> => {
  ...
  if (request.fn === "onsetEnvelope") {
    ...
    if (backend === "gpu") {
      ...
      const {
        mel,
        usedGpu: usedGpuForMel,
        gpuMs: melGpuMs,
        cpuExtraMs: melCpuMs,
      } = await computeMel(true);
  ...
  if (request.fn === "hpssHarmonic" || request.fn === "hpssPercussive") {
    ...
    const needsHpssSpec =
      hpssSpecConfig.fftSize !== specConfig.fftSize ||
      hpssSpecConfig.hopSize !== specConfig.hopSize;
        ...
        return {
          ...
          meta: {
            ...
            timings: {
              totalMs: end - t0,
              cpuMs:
                (needsHpssSpec ? hpssAfterSpec - hpssCpuStart : cpuAfterSpec - cpuStart) +
                (end - hpssStart - out.gpuMs),
              gpuMs: out.gpuMs,
            },
          },
        };
    ...
    const end = nowMs();
    const cpuMs =
      (needsHpssSpec ? hpssAfterSpec - hpssCpuStart : cpuAfterSpec - cpuStart) + (end - hpssStart);
  ...
  if (request.fn === "mfcc" || request.fn === "mfccDelta" || request.fn === "mfccDeltaDelta") {
    ...
    const needsMfccSpec =
      mfccSpecConfig.fftSize !== specConfig.fftSize ||
      mfccSpecConfig.hopSize !== specConfig.hopSize;
  ...
  if (
    request.fn === "cqtHarmonicEnergy" ||
    request.fn === "cqtBassPitchMotion" ||
    request.fn === "cqtTonalStability"
  ) {
