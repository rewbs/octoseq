...
export function logitContributionsByGroupV1(
  ...
  const melContrast = layout.melContrast
    ? sliceDot(w, x, layout.melContrast.offset, layout.melContrast.length)
    : 0;
  const onsetForeground = sliceDot(w, x, layout.onsetFg.offset, layout.onsetFg.length);
  const onsetContrast = layout.onsetContrast
    ? sliceDot(w, x, layout.onsetContrast.offset, layout.onsetContrast.length)
    : 0;
...
export function summariseWeightL2ByGroup(
  w: Float32Array,
  layout: MirFeatureVectorLayoutV1
): MirRefinedModelExplainV1["weightL2"] {
  ...
  const melContrastSq = layout.melContrast
    ? sliceSumSquares(w, layout.melContrast.offset, layout.melContrast.length)
    : 0;
  const onsetForegroundSq = sliceSumSquares(w, layout.onsetFg.offset, layout.onsetFg.length);
  const onsetContrastSq = layout.onsetContrast
    ? sliceSumSquares(w, layout.onsetContrast.offset, layout.onsetContrast.length)
    : 0;
  ...
  const mfcc =
    mfccForegroundSq + mfccContrastSq > 0
      ? Math.sqrt(mfccForegroundSq + mfccContrastSq)
      : undefined;
