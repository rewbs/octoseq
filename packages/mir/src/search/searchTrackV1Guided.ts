...
function decideModelKind(params: { enabled: boolean; labels: MirRefinementCandidateLabelV1[] }): {
  kind: MirRefinedModelKindV1;
  positives: MirRefinementCandidateLabelV1[];
  negatives: MirRefinementCandidateLabelV1[];
} {
...
function cosineSimilarity01ByBlocks(
  ...
  const addBlock = (
    ...
    length: number,
    weight: number,
    acc: { dot: number; aa: number; bb: number }
  ) => {
  ...
  // Foreground blocks
  addBlock(
    layout.melMeanFg.offset,
    layout.melMeanFg.length + layout.melVarianceFg.length,
    wMel,
    acc
  );
  ...
  if (layout.mfccMeanFg && layout.mfccVarianceFg) {
    addBlock(
      layout.mfccMeanFg.offset,
      layout.mfccMeanFg.length + layout.mfccVarianceFg.length,
      wMfcc,
      acc
    );
  }
  ...
  if (layout.onsetContrast)
    addBlock(layout.onsetContrast.offset, layout.onsetContrast.length, wTrans, acc);
  if (layout.mfccMeanContrast && layout.mfccVarianceContrast) {
    addBlock(
      layout.mfccMeanContrast.offset,
      layout.mfccMeanContrast.length + layout.mfccVarianceContrast.length,
      wMfcc,
      acc
    );
  }
...
export async function searchTrackV1Guided(params: {
  ...
  const layout = makeFeatureVectorLayoutV1({
    melDim,
    mfccDim,
    includeContrast: localContrastEnabled,
  });
  ...
  const writeVectorFromState = (opts: {
    ...
    const fgOnsetMax =
      Number.isFinite(fgOnsetMaxRaw) && fgOnsetMaxRaw !== -Infinity ? fgOnsetMaxRaw : 0;
    ...
    if (mfccDim > 0 && mfccFg && mfccBg && layout.mfccMeanFg && layout.mfccVarianceFg) {
      for (let i = 0; i < mfccDim; i++) {
        ...
        if (layout.mfccMeanContrast && layout.mfccVarianceContrast) {
          ...
          const bgVarEx =
            bgExCount > 0
              ? Math.max(0, (bgSumSq - sumSq) / bgExCount - bgMeanEx * bgMeanEx)
              : variance;
  ...
  const buildWindowVectorsPass = (
    onWindow: (
      w: number,
      t0: number,
      t1: number,
      bg: { start: number; end: number },
      vec: Float32Array
    ) => void
  ) => {
    resetSlidingState();
  ...
  let modelMs = 0;
  let trainedModel:
    ReturnType<typeof trainLogisticModelV1> | ReturnType<typeof buildPrototypeModelV1> | null =
    null;
