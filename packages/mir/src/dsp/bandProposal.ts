...
import type { AudioBufferLike } from "./spectrogram";
 ...
 */
function findLocalMaxima(values: Float32Array, minNeighborDistance: number = 3): number[] {
  const peaks: number[] = [];
 ...
 */
function computeTemporalVariance(cqt: CqtSpectrogram, lowBin: number, highBin: number): number {
  const nFrames = cqt.magnitudes.length;
...
function scorePeak(
  ...
  const lowBin = Math.max(
    0,
    Math.floor(peak.lowHz / (cqt.config.fMin * Math.pow(2, 1 / cqt.binsPerOctave)))
  );
 ...
 */
function createBandFromCandidate(candidate: ProposalCandidate, duration: number): FrequencyBand {
...
function filterOverlappingCandidates(
  ...
  for (const candidate of candidates) {
    ...
    for (const existing of filtered) {
      const octaveDiff = Math.abs(Math.log2(candidate.peak.centerHz / existing.peak.centerHz));
