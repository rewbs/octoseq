    ...
    for (const fn of request.functions) {
      switch (fn) {
        ...
        case "bandBeatCandidates": {
          ...
          let onsetPeaksResult = bandEventResults.find((r) => r.fn === "bandOnsetPeaks");
          if (!onsetPeaksResult) {
            // Compute onset peaks if not already done
            onsetPeaksResult = bandOnsetPeaks(sourceSignal, request.onsetPeaksOptions);
          ...
          const result = bandBeatCandidates(onsetPeaksResult, request.beatCandidatesOptions);
          bandEventResults.push(result);
          break;
        }
        default: {
