...
export function validateFrequencySegments(segments: FrequencySegment[]): string[] {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (seg.startTime >= seg.endTime) {
      errors.push(`Segment ${i}: startTime (${seg.startTime}) must be < endTime (${seg.endTime})`);
    }
 ...
 */
export function createStandardBands(
  duration: number,
  sourceId: string = "mixdown"
): FrequencyBand[] {
  const now = new Date().toISOString();
...
export function enabledBandsForSource(
  ...
  if (!structure) return [];
  return sortBands(structure.bands.filter((band) => band.sourceId === sourceId && band.enabled));
}
 ...
 */
export function frequencyBoundsAt(band: FrequencyBand, time: number): FrequencyBoundsAtTime | null {
...
export function updateBandInStructure(
  ...
  return {
    ...structure,
    bands: sortBands(structure.bands.map((b) => (b.id === bandId ? { ...b, ...updates } : b))),
    modifiedAt: new Date().toISOString(),
  };
}
...
export function splitBandSegmentAt(band: FrequencyBand, time: number): FrequencyBand {
  ...
  // Find the segment containing this time
  const segmentIndex = segments.findIndex((seg) => time > seg.startTime && time < seg.endTime);
 ...
 */
export function mergeAdjacentSegments(band: FrequencyBand, tolerance: number = 0.1): FrequencyBand {
  const segments = sortFrequencySegments(band.frequencyShape);
...
export function removeKeyframe(band: FrequencyBand, time: number): FrequencyBand {
  ...
  const endingIndex = segments.findIndex((seg) => Math.abs(seg.endTime - time) < 0.001);
  const startingIndex = segments.findIndex((seg) => Math.abs(seg.startTime - time) < 0.001);
...
export function moveKeyframeTime(
  ...
  const endingIndex = segments.findIndex((seg) => Math.abs(seg.endTime - oldTime) < 0.001);
  const startingIndex = segments.findIndex((seg) => Math.abs(seg.startTime - oldTime) < 0.001);
