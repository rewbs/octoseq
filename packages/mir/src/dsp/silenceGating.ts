...
export function withSilenceGateDefaults(config?: SilenceGateConfig): Required<SilenceGateConfig> {
  return {
    ...
    postSilenceSuppressMs:
      config?.postSilenceSuppressMs ?? DEFAULT_SILENCE_GATE_CONFIG.postSilenceSuppressMs,
  };
}
 ...
 */
export function buildSuppressionMask(activityMask: Uint8Array, suppressFrames: number): Uint8Array {
 ...
 */
export function applyMinActiveDuration(activityMask: Uint8Array, minActiveFrames: number): void {
...
export function computeSilenceGating(
  ...
  const suppressFrames = Math.max(
    0,
    Math.round(cfg.postSilenceSuppressMs / 1000 / frameDurationSec)
  );
  ...
  const activityMask = buildActivityMask(
    frameEnergy,
    enterThreshold,
    exitThreshold,
    hangoverFrames
