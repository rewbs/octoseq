...
function createCqtKernel(
  ...
  const endBin = Math.min(Math.floor(fftSize / 2) + 1, Math.ceil(fHigh / freqResolution) + 1);
 ...
 */
function getCqtKernelBank(config: CqtConfig, fftSize: number, sampleRate: number): CqtKernelBank {
 ...
 */
function applyCqtKernels(stftMagnitudes: Float32Array, kernelBank: CqtKernelBank): Float32Array {
...
export async function cqtSpectrogram(
  ...
  // Compute STFT
  const stft = await spectrogram(audio, { fftSize, hopSize, window: "hann" }, undefined, {
