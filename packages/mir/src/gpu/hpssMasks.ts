...
export async function gpuHpssMasks(
  ...
  const params = createUniformBufferU32x4(
    gpu,
    new Uint32Array([nBins, nFrames, softMask ? 1 : 0, 0])
  );
  ...
  await Promise.all([
    harmonicReadback.mapAsync(GPUMapMode.READ),
    percussiveReadback.mapAsync(GPUMapMode.READ),
  ]);
  const tDone = nowMs();
