...
/// Vertex buffer layout for billboard quad vertices.
/// Each vertex is a 2D position (-0.5 to 0.5).
pub fn billboard_quad_vertex_desc<'a>() -> wgpu::VertexBufferLayout<'a> {
...
/// Uses instanced rendering with two vertex buffers:
/// - Slot 0: Quad vertex data (2D position) with VertexStepMode::Vertex
/// - Slot 1: Instance data (position, scale, color) with VertexStepMode::Instance
pub fn create_billboard_particle_pipeline(
    device: &wgpu::Device,
    layout: &wgpu::PipelineLayout,
    color_format: wgpu::TextureFormat,
) -> wgpu::RenderPipeline {
    let shader = device.create_shader_module(wgpu::include_wgsl!("shader_particle.wgsl"));

    device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: Some("Billboard Particle Pipeline"),
        layout: Some(layout),
        vertex: wgpu::VertexState {
            module: &shader,
            ...
            buffers: &[
                ...
                billboard_quad_vertex_desc(),
                // Slot 1: Instance data (per-instance)
                GpuParticleInstance::desc(),
            ],
            compilation_options: wgpu::PipelineCompilationOptions::default(),
        },
        fragment: Some(wgpu::FragmentState {
        ...
        primitive: wgpu::PrimitiveState {
            ...
            strip_index_format: None,
            front_face: wgpu::FrontFace::Ccw,
            cull_mode: None, // No culling for billboards (always face camera)
            polygon_mode: wgpu::PolygonMode::Fill,
            unclipped_depth: false,
            conservative: false,
        },
