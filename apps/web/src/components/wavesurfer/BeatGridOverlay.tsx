...
export function BeatGridOverlay({
  ...
  const provisionalBeatTimes = useMemo(() => {
    ...
    return generateBeatTimes(beatGrid.bpm, beatGrid.phaseOffset, beatGrid.userNudge, audioDuration);
  ...
  useEffect(() => {
    ...
    const renderSubBeatLine = (time: number, zIndex: number = 0) => {
      ...
      div.style.background =
        "repeating-linear-gradient(to bottom, rgba(156, 163, 175, 0.5) 0px, rgba(156, 163, 175, 0.5) 2px, transparent 2px, transparent 6px)";
      div.style.pointerEvents = "none";
      div.style.zIndex = String(zIndex);
      host.appendChild(div);
    };
    ...
    const renderSegmentBoundary = (time: number, isSelected: boolean, isStart: boolean) => {
      const x = (time - viewport.startTime) * pxPerSec;
      if (x < -10 || x > width + 10) return;
      ...
      marker.style.borderTop =
        "8px solid " + (isSelected ? "rgba(59, 130, 246, 0.9)" : "rgba(139, 92, 246, 0.7)");
