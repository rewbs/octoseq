...
export const WaveSurferPlayer = forwardRef<WaveSurferPlayerHandle, WaveSurferPlayerProps>(
  function WaveSurferPlayer(
    ...
    const segmentPlaybackRef = useRef<{ startSec: number; endSec: number; loop: boolean } | null>(
      null
    );
    const lastPlaybackEmitRef = useRef<{ atMs: number; timeSec: number }>({
      atMs: 0,
      timeSec: -Infinity,
    });
    ...
    const onSelectCandidateIdRef =
      useRef<WaveSurferPlayerProps["onSelectCandidateId"]>(onSelectCandidateId);
    const onManualCandidateCreateRef =
      useRef<WaveSurferPlayerProps["onManualCandidateCreate"]>(onManualCandidateCreate);
    const onManualCandidateUpdateRef =
      useRef<WaveSurferPlayerProps["onManualCandidateUpdate"]>(onManualCandidateUpdate);
    ...
    const onIsPlayingChangeRef =
      useRef<WaveSurferPlayerProps["onIsPlayingChange"]>(onIsPlayingChange);
    ...
    useEffect(() => {
      ...
      const getTimeFromClientX = (clientX: number) => {
        ...
        const x = Math.max(
          0,
          Math.min(totalWidth, clientX - rect.left + scrollContainer.scrollLeft)
        );
      ...
      const setupRegionInteractions = (regionsPlugin: RegionsPluginLike) => {
        ...
        const getRegionById = (id: string | null) =>
          id ? (regionsPlugin.getRegion(id) ?? null) : null;
        ...
        const handlePointerMove = (evt: PointerEvent) => {
          ...
          if (dragMode === "move") {
            ...
            const nextStart = Math.max(
              0,
              Math.min(duration - regionDuration, t - dragPointerOffset)
            );
        ...
        const handlePointerUp = (evt: PointerEvent) => {
          ...
          if (wasCreate && wasDrag) {
            if (addMissingModeRef.current) {
              onManualCandidateCreateRef.current?.({
                id: region.id,
          ...
          if (candidate?.source === "manual") {
            onManualCandidateUpdateRef.current?.({
              id: region.id,
              startSec: region.start,
              endSec: region.end,
            });
      ...
      const initPlugins = async () => {
        try {
          ...
          const regionsPlugin = regionsRegistration.instance.actions as
            RegionsPluginLike | undefined;
          if (regionsPlugin) {
    ...
    useEffect(() => {
      ...
      const getFill = (c: RefinementCandidate): string => {
        if (c.status === "accepted")
          return c.source === "manual" ? "rgba(34, 197, 94, 0.28)" : "rgba(34, 197, 94, 0.22)";
    ...
    return (
      <div className="w-full ">
          ...
          {(activeBeatGrid || selectedHypothesis || musicalTimeStructure) && (
            <div className="flex items-center gap-2 border-l border-zinc-300 dark:border-zinc-700 pl-2 ml-1">
                ...
                {showTimingInfo && (
                  <div className="absolute left-0 top-full mt-1 z-50 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-lg p-3 min-w-[300px]">
                    {(() => {
                      ...
                      const bpm =
                        segment?.bpm ?? activeBeatGrid?.bpm ?? selectedHypothesis?.bpm ?? 0;
              ...
              <select
                ...
                onChange={(e) =>
                  setSubBeatDivision(Number(e.target.value) as typeof subBeatDivision)
                }
            ...
            {activeRegion ? (
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="tabular-nums">
                  frame{" "}
                  {sampleRate && sampleRate > 0 && hopSize > 0
                    ? ((playheadTime * sampleRate) / hopSize).toFixed(2)
                    : "—"}
                </span>
                <span className="text-zinc-400">|</span>
                <span className="tabular-nums">
                  {(() => {
                    ...
                    if (activeBeatGrid) {
                      return (
                        <>
                          ...
                          {(
                            ((playheadTime -
                              (activeBeatGrid.phaseOffset + activeBeatGrid.userNudge)) *
                              activeBeatGrid.bpm) /
                              60 +
                            1
                          ).toFixed(2)}
                    ...
                    if (selectedHypothesis) {
                      return (
                        <>beat {((playheadTime * selectedHypothesis.bpm) / 60 + 1).toFixed(2)}</>
                      );
                    }
                    return "beat —";
                  })()}
        ...
        <div
          className={`mt-1.5 rounded-md border border-zinc-200 bg-white p-1 dark:border-zinc-800 dark:bg-zinc-950 ${
            addMissingMode
        ...
        {!isReady && (
          <div className="mt-2 text-sm">
            {currentAudioSource?.status === "pending" ||
            currentAudioSource?.status === "resolving" ? (
              ...
              <div className="flex items-center gap-2 text-red-600 dark:text-red-400">
                ...
                <span>
                  ...
                  {currentAudioSource.error ? `: ${currentAudioSource.error}` : ""}
                </span>
              </div>
            ) : (
