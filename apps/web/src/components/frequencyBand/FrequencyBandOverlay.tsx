...
import type { FrequencyBandStructure, MelConversionConfig } from "@octoseq/mir";
import {
  frequencyBoundsAt,
  hzToFeatureIndex,
  featureIndexToHz,
...
export function FrequencyBandOverlay({
    ...
    structure.bands.forEach((band, index) => {
      ...
      if (isSelected && band.enabled) {
        ...
        keyframes.forEach((kf) => {
          ...
          const handleRadius = isKeyframeHovered
            ? KEYFRAME_HANDLE_RADIUS + 2
            : KEYFRAME_HANDLE_RADIUS;
          [yHigh, yLow].forEach((y) => {
            ctx.beginPath();
            ctx.arc(x, y, handleRadius, 0, Math.PI * 2);
            ctx.fillStyle = isKeyframeHovered
              ? `rgba(${color.r}, ${color.g}, ${color.b}, 1.0)`
              : "white";
            ctx.fill();
            ctx.strokeStyle = isKeyframeHovered
    ...
    (clientX: number, clientY: number): { bandId: string; time: number } | null => {
      ...
      for (const kf of keyframes) {
        ...
        if (Math.abs(x - kfX) <= KEYFRAME_HANDLE_RADIUS + 3) {
          ...
          if (
            Math.abs(y - yHigh) <= KEYFRAME_HANDLE_RADIUS + 3 ||
  ...
  const hitTestBand = useCallback(
    (
      clientX: number,
      clientY: number
    ): { bandId: string; edge: "low-edge" | "high-edge" | "body" } | null => {
      const canvas = canvasRef.current;
