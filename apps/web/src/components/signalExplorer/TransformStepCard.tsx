...
import type { StepSamples, TransformStep, TransformType } from "@/lib/signalExplorer/types";
...
const transformTypeStyles: Record<TransformType, string> = {
  Source: "bg-blue-500/10 border-blue-500/30 text-blue-700 dark:text-blue-300",
  Smooth: "bg-purple-500/10 border-purple-500/30 text-purple-700 dark:text-purple-300",
  Normalise: "bg-green-500/10 border-green-500/30 text-green-700 dark:text-green-300",
  Gate: "bg-amber-500/10 border-amber-500/30 text-amber-700 dark:text-amber-300",
  Arithmetic: "bg-cyan-500/10 border-cyan-500/30 text-cyan-700 dark:text-cyan-300",
  ...
  ExpLog: "bg-orange-500/10 border-orange-500/30 text-orange-700 dark:text-orange-300",
  Modular: "bg-teal-500/10 border-teal-500/30 text-teal-700 dark:text-teal-300",
  Mapping: "bg-red-500/10 border-red-500/30 text-red-700 dark:text-red-300",
  TimeShift: "bg-indigo-500/10 border-indigo-500/30 text-indigo-700 dark:text-indigo-300",
  RateChange: "bg-rose-500/10 border-rose-500/30 text-rose-700 dark:text-rose-300",
...
export const TransformStepCard = memo(function TransformStepCard({
  ...
  const colorClass = transformTypeStyles[step.transform_type] ?? transformTypeStyles.Debug;
      ...
      <div className="px-2 py-1.5">
        ...
        <div className="flex items-center justify-between gap-2 mb-1">
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="text-xs font-mono truncate" title={step.description}>
              {step.description}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 text-tiny font-mono opacity-70 shrink-0">
            <span title="Current value">{formatNumber(samples.stats.current_value)}</span>
