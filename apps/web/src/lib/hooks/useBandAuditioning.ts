...
export function useBandAuditioning({
  ...
  const [currentBounds, setCurrentBounds] = useState<{ lowHz: number; highHz: number } | null>(
    null
  );

  // Get the soloed band
  const soloedBand: FrequencyBand | null = soloedBandId
  ...
  // Play control
  const play = useCallback((fromTimeSec?: number) => {
