...
import type { MirAudioPayload, MirBackend, MirResult, MirRunRequest } from "@octoseq/mir";
...
export class MirWorkerClient {
  ...
  constructor() {
    ...
    this.worker = new Worker(new URL("../workers/mirWorker.ts", import.meta.url), {
      type: "module",
    });
  }
  ...
  run(audio: MirAudioPayload, request: MirRunRequest, opts: MirWorkerClientOptions): MirWorkerJob {
    ...
    const promise = new Promise<MirResult>((resolve, reject) => {
      ...
      const onMessage = (ev: MessageEvent<MirWorkerOutMessage>) => {
        ...
        if (m.result.kind === "activity") {
          resolve({
            kind: "activity",
            times,
            activityLevel: new Float32Array(m.result.activityLevel ?? new ArrayBuffer(0)),
            isActive: new Uint8Array(m.result.isActive ?? new ArrayBuffer(0)),
            suppressMask: new Uint8Array(m.result.suppressMask ?? new ArrayBuffer(0)),
            diagnostics: m.result.activityDiagnostics ?? {
              noiseFloor: 0,
              enterThreshold: 0,
              exitThreshold: 0,
              totalFrames: 0,
        ...
        const data = (m.result.data2d ?? []).map(
          (b: ArrayBufferLike) => new Float32Array(b as ArrayBuffer)
        );
        resolve({
          kind: "2d",
          times,
