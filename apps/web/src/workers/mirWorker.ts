...
function serialiseResult(
  ...
  if (result.kind === "activity") {
    const activityLevel = result.activityLevel.buffer.slice(
      result.activityLevel.byteOffset,
      result.activityLevel.byteOffset + result.activityLevel.byteLength
    ) as ArrayBuffer;
    const isActive = result.isActive.buffer.slice(
      result.isActive.byteOffset,
      result.isActive.byteOffset + result.isActive.byteLength
    ) as ArrayBuffer;
    const suppressMask = result.suppressMask.buffer.slice(
      result.suppressMask.byteOffset,
      result.suppressMask.byteOffset + result.suppressMask.byteLength
    ) as ArrayBuffer;
    transfer.push(activityLevel, isActive, suppressMask);
    ...
    return {
      msg: {
        ...
        result: {
          ...
          isActive,
          suppressMask,
          activityDiagnostics: result.diagnostics,
          meta: result.meta,
        },
      },
      transfer,
...
async function handleRun(m: MirWorkerRunMessage) {
  ...
  postLog(jobId, "info", "RUN", {
    fn: m.request.fn,
    backend: m.request.backend,
    enableGpu: m.enableGpu,
  });
...
async function handleSearch(m: MirWorkerSearchMessage) {
  ...
  try {
    ...
    if (useGpu && gpuCtx) {
      try {
        const onsetGpu = await onsetEnvelopeFromMelGpu(mel, gpuCtx, {
          diffMethod: m.features?.onset?.diffMethod,
        });
    ...
    const mfccFeatures = mfccResult
      ? { times: mfccResult.times, values: mfccResult.coeffs }
      : undefined;
...
ctx.onmessage = (ev: MessageEvent<MirWorkerInMessage>) => {
  ...
  if (msg.type === "CANCEL") {
    ...
    // Also bound memory if a cancellation arrives after a job already ended.
    setTimeout(() => cancelled.delete(msg.jobId), 60_000);
  ...
  if (msg.type === "INIT") {
    ...
    if (msg.enableGpu) {
      void ensureGpu()
        ...
        .catch((e) =>
          postLog(undefined, "error", "GPU init failed", { message: (e as Error).message })
        );
    }
    return;
  }
